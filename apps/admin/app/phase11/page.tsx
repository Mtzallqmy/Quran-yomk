...
function ScheduleEditor({virtualData,session,reload}:{virtualData:any;session:Session;reload:()=>Promise<void>}){
  ...
  const stationMap=useMemo(()=>new Map<string,AnyRow>((Array.isArray(virtualData?.stations)?virtualData.stations:[]).map((s:AnyRow)=>[String(s.id),s] as [string,AnyRow])),[virtualData]);
