...
export function publicEnv():PublicEnv{
  ...
  const environment=process.env.TARTEEL_ENVIRONMENT??'development';
  const secureCookies=process.env.TARTEEL_COOKIE_SECURE==null?environment==='production':process.env.TARTEEL_COOKIE_SECURE==='true';
  return {url,publishable,environment,streamBase:(process.env.TARTEEL_PUBLIC_STREAM_BASE_URL??'').replace(/\/$/,''),mount:process.env.TARTEEL_INTERNAL_MOUNT??'/tarteel.mp3',secureCookies};
}
...
export async function authSignOut(accessToken:string){const e=env();const r=await fetch(`${e.url}/auth/v1/logout?scope=local`,{method:'POST',headers:{apikey:e.publishable,authorization:`Bearer ${accessToken}`},cache:'no-store'});if(!r.ok)throw new ApiError(502,'SESSION_REVOCATION_FAILED','Could not revoke the current session');}
