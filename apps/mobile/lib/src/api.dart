...
class TarteelApiClient {
  ...
  Future<JsonMap> _get(String path, {Map<String, String?> query = const <String, String?>{}, bool allowRetry = true}) async {
    ...
    for (var attempt = 0; attempt < attempts; attempt++) {
      try {
        ...
        if (response.statusCode >= 200 && response.statusCode < 300 && body is Map) return Map<String, dynamic>.from(body);
        final error = body is Map ? jsonMap(body['error']) : const <String, dynamic>{};
  ...
  Future<PageResult<Station>> stations({int page = 1, int limit = 50}) async {
    final root = await _get('stations', query: <String, String?>{'page': '$page', 'limit': '$limit'});
