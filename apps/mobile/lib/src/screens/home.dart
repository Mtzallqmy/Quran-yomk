...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  Future<void> refresh() async {
    ...
    setState(() {
      future = Future.wait<dynamic>(<Future<dynamic>>[
        repo.featured(refresh: true), repo.stations(refresh: true), repo.reciters(refresh: true), repo.categories(refresh: true), repo.appConfig(refresh: true),
      ]).then((values) => HomeData(featured: values[0], stations: values[1], reciters: values[2], categories: values[3], config: values[4]));
    });
    await future;
  }
  ...
  Widget build(BuildContext context) => FutureBuilder<HomeData>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError && !snapshot.hasData) return ErrorPane(error: snapshot.error!, onRetry: refresh);
          ...
          return RefreshIndicator(
            ...
            child: ListView(children: <Widget>[
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return SizedBox(
                        ...
                        child: Card(
                          child: InkWell(
                            onTap: item.slug == null ? null : () async {
                            ...
                            child: Padding(
                              ...
                              child: Row(children: <Widget>[
                                ...
                                Expanded(child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                                  Text(item.nameAr, maxLines: 2, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.titleMedium),
                                  const SizedBox(height: 6),
                                  const Row(children: <Widget>[Icon(Icons.play_circle_outline, size: 18), SizedBox(width: 4), Text('تشغيل')]),
                                ])),
                              ]),
                            ),
                          ),
                    ...
                    return SizedBox(
                      ...
                      child: Card(
                        child: ListTile(
                          ...
                          title: Text(station.nameAr, maxLines: 2, overflow: TextOverflow.ellipsis),
                          subtitle: Text(station.isInternal ? 'ترتيل الداخلي' : 'محطة خارجية'),
                          trailing: IconButton(icon: const Icon(Icons.play_arrow), onPressed: station.isPlayable ? () => services.playback.playStation(station) : null),
                        ),
                      ),
                    );
                  },
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return SizedBox(
                      ...
                      child: Card(
                        child: ListTile(
                          leading: Artwork(url: reciter.imageUrl, size: 46, icon: Icons.person_outline),
                          title: Text(reciter.nameAr, maxLines: 2, overflow: TextOverflow.ellipsis),
              ...
              Padding(
                ...
                child: Wrap(spacing: 8, runSpacing: 8, children: data.categories.take(12).map((category) => Chip(label: Text(category.nameAr))).toList(growable: false)),
              ),
            ]),
          );
        },
