...
> **Phase 6:** الانتقالات AUTO/SCHEDULED/MANUAL أصبحت منفذة، وLiquidsoap يبقى متصلًا بالم mount الثابت أثناء تغيير القرار. التحكم runtime عبر loopback-only command socket؛ `main.reload` يطبق Queue عند boundary و`main.skip` يستخدم فقط لـINTERRUPT/SKIP الصريح. راجع [`QUEUE_MANAGER.md`](./QUEUE_MANAGER.md) و[`RADIO_COMMANDS.md`](./RADIO_COMMANDS.md).
