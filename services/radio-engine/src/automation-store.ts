...
export type CommandEffectStatus='PREPARED'|'DISPATCHED'|'ACKED'|'FAILED';
...
export class SupabaseAutomationStore {
  ...
  async recordCommandEffect(lease:Lease,commandId:string,effectType:string,payloadHash:string,status:CommandEffectStatus,result:Record<string,unknown>={}):Promise<string>{const {data,error}=await this.client.schema('radio').rpc('record_command_effect',{p_command_id:commandId,p_station_id:lease.stationId,p_owner_id:lease.ownerId,p_fencing_token:lease.fencingToken,p_effect_type:effectType,p_payload_hash:payloadHash,p_status:status,p_result:result});if(error)throw error;return String(data);}
