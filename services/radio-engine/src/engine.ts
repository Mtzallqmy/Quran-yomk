...
export class RadioEngine {
  ...
  private stopping=false;private heartbeat?:NodeJS.Timeout;private timeline?:NodeJS.Timeout;private tracks:Track[]=[];private trackIndex=0;
  private restartCount=0;private workspace='';
  readonly snapshot:EngineSnapshot={mode:'STOPPED',sourceConnected:false,streamMount:'',current:null,next:null,currentStartedAt:null,expectedEndAt:null,sourceStartedAt:null,lastError:null,lastRecoveryAt:null,reconnectCount:0,trackFailures:0};
  ...
  async start():Promise<void>{
    ...
    this.source=new LiquidsoapSource(this.config,playlistPath,scriptPath);await this.source.prepare();this.server=await startHealthServer(this.config.healthPort,()=>this.snapshot,()=>this.snapshot.mode==='AUTO'&&this.snapshot.sourceConnected);
  ...
  private startSource():void{
    ...
    this.source.start((code,signal)=>void this.onSourceExit(code,signal),line=>this.logger.info('SOURCE_LOG',{message:line}),event=>this.onSourceEvent(event));
  ...
  private onSourceEvent(event:string):void{if(event==='SOURCE_CONNECTED'){this.restartCount=0;this.snapshot.sourceConnected=true;this.snapshot.lastError=null;if(this.snapshot.mode==='STARTING'||this.snapshot.mode==='RECOVERING')this.transition('AUTO');this.logger.info('SOURCE_CONNECTED',{station_id:this.config.stationId,mount:this.config.mount,pid:this.source?.pid});if(!this.timeline)this.startTimeline();void this.checkpoint();}else if(event==='SOURCE_DISCONNECTED'||event==='SOURCE_ERROR'){this.snapshot.sourceConnected=false;this.snapshot.lastError=event;this.snapshot.lastRecoveryAt=new Date().toISOString();if(this.snapshot.mode==='AUTO')this.transition('RECOVERING');this.logger.warn(event,{mount:this.config.mount});void this.checkpoint();}}
  private startTimeline():void{if(this.timeline)clearTimeout(this.timeline);const track=this.tracks[this.trackIndex];if(!track)return;const next=this.tracks[(this.trackIndex+1)%this.tracks.length]??null;
    this.snapshot.current=track;this.snapshot.next=next;const now=Date.now();this.snapshot.currentStartedAt=new Date(now).toISOString();this.snapshot.expectedEndAt=new Date(now+track.durationSeconds*1000).toISOString();
    this.logger.info('TRACK_START',{media_id:track.mediaId,title:track.title,duration_seconds:track.durationSeconds});void this.updateIcecastMetadata(track);void this.checkpoint();
    this.timeline=setTimeout(()=>{this.logger.info('TRACK_END',{media_id:track.mediaId,title:track.title});this.trackIndex=(this.trackIndex+1)%this.tracks.length;this.startTimeline();},Math.max(100,track.durationSeconds*1000));this.timeline.unref();}
  ...
  private async onSourceExit(code:number|null,signal:NodeJS.Signals|null):Promise<void>{if(this.stopping)return;this.snapshot.sourceConnected=false;this.snapshot.lastError=`source exited code=${code} signal=${signal}`;this.logger.warn('SOURCE_DISCONNECTED',{code,signal});if(this.timeline)clearTimeout(this.timeline);
  ...
  private async heartbeatTick():Promise<void>{if(!this.lease||this.stopping)return;try{this.lease=await this.store.renew(this.lease,this.config.leaseSeconds);await this.checkpoint();}catch(error){this.logger.error('ENGINE_OWNERSHIP_LOST',error);this.snapshot.sourceConnected=false;this.snapshot.lastError='station lease lost';this.source?.stop('SIGTERM');if(this.snapshot.mode!=='ERROR')this.transition('ERROR');}}
  ...
  async stop():Promise<void>{if(this.stopping)return;this.stopping=true;if(this.heartbeat)clearInterval(this.heartbeat);if(this.timeline)clearTimeout(this.timeline);this.source?.stop('SIGTERM');this.snapshot.sourceConnected=false;if(this.snapshot.mode!=='STOPPED')this.transition('STOPPED');await this.checkpoint().catch(()=>{});if(this.lease)await this.store.release(this.lease).catch(()=>{});await new Promise<void>(resolve=>this.server?this.server.close(()=>resolve()):resolve());await rm(this.workspace,{recursive:true,force:true});this.logger.info('ENGINE_STOP',{station_id:this.config.stationId});}
}
