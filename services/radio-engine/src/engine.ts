...
export class RadioEngine {
  ...
  private handleTrackStart(payload?:string):void{
    ...
    try{const metadata=JSON.parse(payload??'{}') as Record<string,unknown>;const rawIndex=metadata.tarteel_index;const parsed=typeof rawIndex==='number'?rawIndex:typeof rawIndex==='string'&&/^\d+$/.test(rawIndex)?Number(rawIndex):NaN;if(Number.isInteger(parsed)&&parsed>=0)acknowledgedIndex=parsed;const queue=String(metadata.queue_entry_id??'');if(queue&&queue!=='none')queueEntryId=queue;const media=String(metadata.media_id??'');if(media&&media!=='none')mediaId=media;}catch{}
    ...
    if(queueEntryId){track=this.automationTracks.find(value=>value.queueEntryId===queueEntryId);if(track){const index=this.automationTracks.indexOf(track);next=this.automationTracks[index+1]??null;}}
    else if(acknowledgedIndex!==undefined){const index=acknowledgedIndex;track=this.mainTracks[index];if(track){this.mainTrackIndex=(index+1)%this.mainTracks.length;next=this.mainTracks[this.mainTrackIndex]??null;}}
  ...
  async applyAutomationTracks(tracks:Track[],interrupt=false):Promise<void>{if(!this.source||tracks.length===0)throw new Error('radio source is not ready');const activeSource=this.snapshot.current?.queueEntryId?'automation':'main';await this.source.clearAutomation();this.automationTracks=[...tracks];for(const [index,track] of tracks.entries())await this.source.pushTrack(track.path,track.mediaId,track.queueEntryId??null,interrupt&&index===0,activeSource);this.logger.info('QUEUE_CHANGED',{track_count:tracks.length,interrupt,control:'request.queue',active_source:activeSource});}
