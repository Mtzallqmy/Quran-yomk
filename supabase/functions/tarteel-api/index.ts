...
function uuidList(value: string | null) {
  ...
  if (
    values.length > 8 ||
    values.some((item) => !/^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(item))
  ) {
    throw Object.assign(new Error("Invalid failed station ids"), {
      status: 422,
    });
  }
  return values;
...
async function catalog(
  ...
  const rows = await rpc(
    "tarteel_public_station_catalog",
    {
      p_environment:
        Deno.env.get("TARTEEL_PUBLIC_ENVIRONMENT") ?? "development",
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method !== "GET") {
    return fail(
      405,
      "METHOD_NOT_ALLOWED",
      "Only public GET endpoints are exposed",
      requestId,
    );
  }
  ...
  if (!apiKey || !ALLOWED_KEYS.has(apiKey)) {
    return fail(
      401,
      "INVALID_API_KEY",
      "A valid Tarteel publishable API key is required",
      requestId,
    );
  }
  if (!SUPABASE_URL || !INTERNAL_API_KEY) {
    return fail(
      ...
      "Public API runtime is not configured",
      requestId,
    );
  }
  ...
  try {
    ...
    const quran = await handleQuranApi(
      parts,
      u.searchParams,
      async () =>
        asArray(await rpc("tarteel_public_surahs", {}, apiKey)) as Record<
          string,
          unknown
        >[],
    );
    if (quran) {
      return response({ data: quran.data }, 200, {
        "x-request-id": requestId,
        "cache-control": quran.cacheControl,
      });
    }
    ...
    if (parts[0] === "virtual-radio") {
      ...
      console.info(JSON.stringify({
        ...
        available,
        station_id:
          available && data?.station && typeof data.station === "object"
      ...
      if (!available) {
        const code = String(
          data?.error_code ?? "NO_VIRTUAL_SOURCE_AVAILABLE",
        );
        return response(
          {
            data,
            error: {
              code,
              message: "No Tarteel Radio source is available right now",
              request_id: requestId,
            },
          },
          503,
    ...
    if (parts[0] === "stations") {
      ...
      if (!station) {
        return fail(404, "NOT_FOUND", "Station not found", requestId);
      }
      if (parts.length === 2) {
        return response({ data: station }, 200, {
          "x-request-id": requestId,
          "cache-control": "public, max-age=30, s-maxage=300",
        });
      }
      if (parts[2] === "offline-clip-policy") {
        const data = await rpc(
          "tarteel_public_offline_clip_policy",
    ...
    if (parts[0] === "content-sources" && parts.length === 1) {
      return response(
        { data: asArray(await rpc("tarteel_public_content_sources", {}, apiKey)) },
        200,
        {
          "x-request-id": requestId,
    ...
    if (parts[0] === "categories" && parts.length === 1) {
      return response(
        { data: asArray(await rpc("tarteel_public_categories", {}, apiKey)) },
        200,
        {
          "x-request-id": requestId,
          "cache-control": "public, max-age=300, s-maxage=3600",
    ...
    if (parts[0] === "reciters") {
      ...
      if (parts.length === 1) {
        const data = asArray(
          await rpc(
            "tarteel_public_reciters",
            {
              p_search: (u.searchParams.get("q") ?? "").trim() || null,
              p_limit: limit,
        ...
        return response(
          {
            ...
            limit,
            total: data.length,
            next_page: data.length === limit ? page + 1 : null,
          },
          200,
          {
            "x-request-id": requestId,
      ...
      if (!/^[0-9a-f-]{36}$/i.test(id)) {
        return fail(422, "VALIDATION_ERROR", "Invalid reciter id", requestId);
      }
      if (parts.length === 2) {
        const rows = asArray(
          await rpc(
            "tarteel_public_reciters",
            { p_search: null, p_limit: 1, p_offset: 0, p_id: id },
            apiKey,
          ),
        );
        if (!rows[0]) {
          return fail(404, "NOT_FOUND", "Reciter not found", requestId);
        }
        return response({ data: rows[0] }, 200, {
          "x-request-id": requestId,
        });
      }
      if (parts[2] === "surahs") {
    ...
    if (parts[0] === "search" && parts.length === 1) {
      ...
      const stations = (
        await catalog(new URLSearchParams({ search: q, limit: "10" }), apiKey)
      ).rows;
      const [reciterRaw, surahRaw] = await Promise.all([
        rpc(
          "tarteel_public_reciters",
          { p_search: q, p_limit: 10, p_offset: 0, p_id: null },
          apiKey,
        ),
        rpc("tarteel_public_surahs", {}, apiKey),
      ]);
      const normalized = q.toLowerCase();
      const surahs = asArray(surahRaw)
        .filter((item) => {
          const s = item as Record<string, unknown>;
          return String(s.name_ar ?? "").includes(q) ||
            String(s.name_en ?? "").toLowerCase().includes(normalized);
        })
        .slice(0, 10);
      return response(
        { data: { stations, reciters: asArray(reciterRaw), surahs } },
        200,
        {
          "x-request-id": requestId,
          "cache-control": "public, max-age=0, s-maxage=3",
    ...
    if (status === 401 || status === 403) {
      return fail(
        status,
        "UPSTREAM_AUTH_ERROR",
        "Public data authorization failed",
        requestId,
      );
    }
    if (status === 404) {
      return fail(404, "NOT_FOUND", "Resource not found", requestId);
    }
    if (status === 422) {
      return fail(
        422,
