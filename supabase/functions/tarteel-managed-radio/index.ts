...
}catch(error){const e=error instanceof ManagedRadioError?error:new ManagedRadioError("INTERNAL_ERROR","Unexpected managed radio error",500);console.error(JSON.stringify({event:"MANAGED_RADIO_ERROR",request_id:requestId,code:e.code,status:e.status,message:e.message}));return out({error:{code:e.code,message:e.message,request_id:requestId}},e.status,requestId)}});
