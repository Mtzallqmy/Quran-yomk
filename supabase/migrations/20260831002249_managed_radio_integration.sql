...
declare
  v_base jsonb;
  v_cfg app.managed_radio_configs%rowtype;
  v_channel_id uuid;
  v_stream text;
...
begin
  ...
  if v_channel_id is not null then
    select * into v_cfg from app.managed_radio_configs where channel_id=v_channel_id;
  end if;
  ...
  if v_managed then
    ...
    v_base := jsonb_set(v_base,'{playback}',jsonb_build_object(
      'url',v_stream,'kind','MANAGED_RADIO','is_live',true,'seekable',false,'provider',v_cfg.provider
    ),true);
  else
    v_base := jsonb_set(v_base,'{mode}','"DIRECT_FALLBACK"'::jsonb,true);
    if coalesce(v_base->>'available','false')='true' then
      v_base := jsonb_set(v_base,'{playback}',jsonb_build_object(
        'url',v_base#>>'{station,playback_url}','kind','EXTERNAL_DIRECT','is_live',true,'seekable',false
      ),true);
    end if;
  end if;

  v_base := jsonb_set(v_base,'{managed_radio}',jsonb_build_object(
    'provider',coalesce(v_cfg.provider,'RADIO_CO'),
    'enabled',coalesce(v_cfg.enabled,false),
    'configured',(v_cfg.station_external_id is not null and v_stream is not null),
    'station_external_id',v_cfg.station_external_id,
    'fixed_stream_url',v_stream,
    'backup_playlist_external_id',v_cfg.backup_playlist_external_id,
    'provider_status',v_cfg.provider_status,
    'now_playing',coalesce(v_cfg.provider_now_playing,'{}'::jsonb),
    'last_provider_check_at',v_cfg.last_provider_check_at,
    'last_sync_at',v_cfg.last_sync_at,
    'last_sync_status',coalesce(v_cfg.last_sync_status,'NEVER'),
    'last_sync_error_code',v_cfg.last_sync_error_code
  ),true);
  return v_base;
end;
$$;
